...
/**
 * AIrirang Builder — Public Type Definitions.
 *
 * 모든 모듈(voxelizer · greedy-meshing · palette · datapack · presets · mcp)이
 * 공통으로 import 하는 핵심 타입을 정의합니다. 외부 라이브러리 사용자는
 * `import type { Preset, Voxel, Cuboid, ... } from 'airirang-builder'` 로
 * 동일한 타입을 그대로 가져다 씁니다.
 *
 * Shared type contracts consumed by every module (voxelizer, greedy-meshing,
 * palette, datapack, presets, mcp) and re-exported as the package's public
 * type surface so external consumers can `import type { Preset, ... } from
 * 'airirang-builder'`.
 *
